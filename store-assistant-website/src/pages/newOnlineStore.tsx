...
export default function NewStore({ user }: { user: User }) {
    ...
    const createStore = api.onlineStore.createStore.useMutation()
    ...
    const submitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        await createStore.mutateAsync(body)
            ...
            <div className="bg-slate-50 w-full flex justify-center items-center p-5">
                ...
                <form onSubmit={submitHandler} className={`${raleway.variable} font-sans flex flex-col max-[1920px]:w-2/3 w-1/2 bg-white p-10 h-5/12 shadow-md hover:shadow-lg duration-300 rounded-md gap-7`}>
                    <div className="flex-none flex flex-col w-full items-center gap-5">
                        <p className="text-5xl font-semibold text-slate-600 font-medium text-center">Create a New Online Store Assistant:</p>
                        <div className="h-2"/>
                    </div>
                    <div className="grow flex flex-col items-center justify-center gap-6">
                        ...
                        <input type="text" placeholder="Website:" value={loc} onChange={e => setLoc(e.target.value)} className="w-[60%] border-2 duration-300 border-slate-200 p-5 hover:border-slate-400"/>
                        <textarea placeholder={`Prompt: Include an item and its corresponding description on each line (separated by line)\n\nExample: Item 1: does this thing and that\nItem 2: does this other thing and that other thing`} rows={4} cols={30} value={prompt} onChange={e => setPrompt(e.target.value)} className="w-[60%] border-2 duration-300 border-slate-200 p-5 hover:border-slate-400"/>
                    </div>
